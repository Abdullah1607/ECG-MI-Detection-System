...
Evaluates the trained model on the validation set and prints four
threshold candidates so that THRESHOLD_1LEAD in inference/predict.py
can be updated before running eval_test.py.
...
# Config
# ---------------------------------------------------------------------------
MODEL_PATH = "model/ecg_cnn_500hz.pth"
DATA_DIR   = "data/processed_1lead_500hz"
...
def find_sensitivity_threshold(probs, labels, min_sensitivity):
    fpr, tpr, thresholds = roc_curve(labels, probs)
    # Highest threshold that still achieves >= min_sensitivity
...
def main():
    ...
    print("\nUpdate THRESHOLD_1LEAD in inference/predict.py with the chosen value.")
